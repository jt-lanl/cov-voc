...
def commonforms_args(argparser):
    ...
    paa("--npatterns", "-n", type=int, default=0, help="How many of the most common patterns per lineage (0=all)")
    paa("--mincount", "-m", type=int, default=10, help="Show only patterns with at least this many counts")
    paa("--protein", default="Spike", help="Protein name to be used in the header")
    paa(
        "--baseline",
        default="XBB.1.5",
        type=str.upper,
    ...
    paa("--bylineage", action="store_true", help="Partition sequences by pango lineage")
    paa("--notbylineage", action="store_false", dest="bylineage", help="Do not partition sequences by pango lineges")
...
class LineagePartition:
    ...
    def __init__(self, seqs, bylineage=True, restrict_to=None):
        ...
        v.vprint("LineagePartition: total =", total)
        self.counts = {lin: len(self.sequences[lin]) for lin in self.sequences}
