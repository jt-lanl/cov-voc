...
def ssmlabel(ssmlist,maxlen=20):
    ...
    nonplus = [ssm for ssm in ssmlist if ssm[0]!='+']
    if nonplus:
        site = re.sub('[A-Z]*(\d+).*',r'\1',nonplus[0])
        initletter = nonplus[0][0] + site + ": "
    else:
        initletter = ''
...
def match_column_name(true_names,input_name):
        ...
        if len(the_matches) == 0:
            v.vprint(f'No match found for name={input_name}')
            return None
        if len(the_matches) > 1:
    ...
    def plotsites(self,sitelist):
        ...
        for ndx,site in enumerate(sitelist):
            ...
            ssms = sorted(self.ssms[site],
                          ...
                          reverse=True)
            ssms = [ssm for ssm in ssms
                    if (not np.isnan(self.ssmvals[site].get(ssm)) and
                        not 0 ==    self.ssmvals[site].get(ssm))]
